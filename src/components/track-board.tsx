...
type BoardTrack = Track & { client_name: string; client_slug: string; project_id: string; project_name: string; unchanged_days: number };
...
export function TrackBoard({ tracks, showAll, filtered }: { tracks: BoardTrack[]; showAll: boolean; filtered: boolean }) {
  ...
  function change(track: BoardTrack, status: TrackStatus) {
    ...
    start(async () => {
      try {
        ...
        setError(false);
        setMessage(`${track.client_name} · ${track.project_name} · ${track.name}: ${statusLabel(status)}.`);
      } catch {
        setError(true);
  ...
  function control(track: BoardTrack) {
    return <label className="board-status-control"><span className="sr-only">Status for {track.client_name} · {track.project_name} · {track.name}</span>
  ...
  return <>
      ...
      {columns.map(status => {
        ...
        return <section key={status} aria-label={statusLabel(status)} className={'board-column'+(over === status ? ' is-over' : '')}
          ...
          {cards.map(track => <article key={track.id} className={'board-card'+(dragging === track.id ? ' is-dragging' : '')}>
            <div className="board-card-top"><Link href={'/admin/clients/'+track.client_id+'?project='+track.project_id+'#tracks'} className={filtered ? 'muted' : 'board-client-name'}>{track.client_name} · {track.project_name}</Link>
              <button type="button" className="drag-handle" disabled={!ready || pending} draggable={ready && !pending} aria-label={'Drag '+track.client_name+' '+track.project_name+' '+track.name+' to change status'}
            ...
            <h3><Link href={'/admin/clients/'+track.client_id+'?project='+track.project_id+'#track-'+track.id}>{track.name}</Link></h3>
    ...
    {ongoing.length > 0 && <section className="board-ongoing"><h2><Repeat2 size={19} aria-hidden="true" /> Ongoing</h2><p className="muted">Services that continue beyond launch.</p>{ongoing.map(track => <article key={track.id} className="ongoing-row">
      <div><Link href={'/admin/clients/'+track.client_id+'?project='+track.project_id+'#track-'+track.id}><strong>{track.client_name} · {track.project_name}</strong><span className="muted"> · {track.name}</span></Link><p className="muted">{track.deliverable}{track.status_note && <> · {track.status_note}</>}</p></div>
