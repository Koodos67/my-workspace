...
export function ApprovalResponse({ requestId, kind, consent }: { requestId: string; kind: ApprovalKind; consent: string }) {
  ...
  return <form className="approval-response" onSubmit={event => {
    ...
    <fieldset disabled={!ready || pending}>
      ...
      <div className="approval-buttons"><button className="button" name="decision" value="approved">{kind === 'plan' ? 'Approve plan' : 'Authorise launch'}</button><button className="button secondary" name="decision" value="changes_requested">Request changes</button></div>
    </fieldset>
