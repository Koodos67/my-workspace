...
export async function readApprovals(db: PoolClient, clientId: string): Promise<Approval[]> {
  const { rows } = await db.query(`SELECT a.*,
    ...
    WHERE a.client_id=$1 ORDER BY a.request_number DESC,a.requested_at DESC,a.id`, [clientId]);
