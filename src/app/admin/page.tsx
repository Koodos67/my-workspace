...
export default async function Admin({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const clients = await withActor(profile.id, async db => (await db.query("SELECT c.id,c.name,c.slug,c.status,c.accent_color,(SELECT count(*) FROM memberships m WHERE m.client_id=c.id) AS members,(SELECT count(*) FROM projects p WHERE p.client_id=c.id AND p.archived_at IS NULL) AS projects,(SELECT count(*) FROM items i JOIN projects p ON p.id=i.project_id WHERE i.client_id=c.id AND i.archived_at IS NULL AND p.archived_at IS NULL) AS items FROM clients c ORDER BY c.name")).rows);
  const active=clients.filter(c=>c.status==='active'),archived=clients.filter(c=>c.status==='archived');
  return <Shell signedIn admin name={profile.full_name || 'Admin'} initials="MT"><div className="heading"><div><div className="eyebrow">A home for the work</div><h1>Your clients, together.</h1><p className="muted">A clear view of the people and projects you’re looking after.</p></div><a className="button" href="#new-client">+ New client</a></div><h2>Client directory</h2>{active.length?active.map(client=><Link className="client-row" key={client.id} href={'/admin/clients/'+client.id}><div className="client-icon" style={{borderBottom:'3px solid '+(client.accent_color || '#354c37')}}>{client.name.slice(0,1)}</div><div><h2>{client.name}</h2><p>{client.projects} project{Number(client.projects)===1?'':'s'} · {client.members} members · {client.items} items</p></div><span>Manage workspace ↗</span></Link>):<div className="empty"><h2>Room for your first client.</h2><p className="muted">Create a client below to get started.</p></div>}
