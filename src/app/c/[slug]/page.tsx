...
export default async function ClientWorkspace({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ project?: string }> }) {
  ...
  const data = await withActor(profile.id, async db => {
    ...
    const projects = await readProjects(db, client.id);
    const project = query.project ? projects.find(p => p.id === query.project) : projects[0];
    if (query.project && !project) return null;
  ...
  return <Shell signedIn client name={client.name} initials={(profile.full_name || profile.email).slice(0,2).toUpperCase()}>
    ...
    {profile.role==='admin' && <p><Link className="button secondary" href={'/admin/clients/'+client.id+(data.project ? '?project='+data.project.id : '')}>Manage workspace</Link> <span className="muted">This view shows published content only.</span></p>}
    <ProjectNavigation projects={data.projects} selected={data.project} base={'/c/'+slug} />
    {data.project && <>
